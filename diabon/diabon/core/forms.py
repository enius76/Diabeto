...
class GlycForm(forms.Form):
	value = forms.FloatField(widget=forms.TextInput(attrs={'placeholder': 'ex : 112'}), label =_("Valeur (en mg/dl)"))
	date  = forms.DateTimeField(widget=SelectDateWidget, label =_("Date"))
	time  = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'ex : 12:30'}), max_length=6, label =_("Heure"))
	note  = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Commentaire...'}), max_length=150, label =_("Note"))